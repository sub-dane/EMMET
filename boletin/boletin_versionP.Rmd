...
header-includes: 
  ...
  - \fancyhead[R]{\includegraphics[width=18cm]{img/header.png}\\      
    {\fontfamily{put}\selectfont      \textcolor{gray}{\Large `r paste(params$month_b,params$year)`}}}  
...
```{r variacion,echo=F,eval=TRUE,warning=FALSE,message=F,fig.align='center',fig.height=3.2,fig.width=4}
#1
variacion <- data %>% 
  group_by(ANIO,MES) %>% 
  summarise(produccion=sum(PRODUCCIONREALPOND),
              ventas = sum(VENTASREALESPOND),
              personas = sum( TOTPERS)) #sum(TOTALEMPLEOPERMANENTE+TOTALEMPLEOTEMPORAL+TOTALEMPLEOADMON+TOTALEMPLEOPRODUC))
...
\textbf{Enero 2020 – `r paste0(meses_b[params$month]," ",params$year,"p")`}\

```{r historico,echo=F,eval=TRUE,warning=FALSE,message=F,fig.align='center',fig.height=3,fig.width=5}
#2
...
ggplot(data=variaciones_totales,aes(x=periodo,y=value,group=variables,color=variables))+
  ...
  geom_text(data=variaciones_totales[variaciones_totales$periodo%in%paste0(meses[params$month],"-",substr(params$year,3,4)) & variaciones_totales$variables=="Producción Real",],aes(x=periodo,y=0.15,label=round(value*100,1),color=variables),show.legend = F,hjust=-0.2,size=2.8)+
    geom_text(data=variaciones_totales[variaciones_totales$periodo%in%paste0(meses[params$month],"-",substr(params$year,3,4)) & variaciones_totales$variables=="Ventas Reales",],aes(x=periodo,y=0.10,label=round(value*100,1),color=variables),show.legend = F,hjust=-0.2,size=2.8)+
    geom_text(data=variaciones_totales[variaciones_totales$periodo%in%paste0(meses[params$month],"-",substr(params$year,3,4)) & variaciones_totales$variables=="Personal Ocupado",],aes(x=periodo,y=0.05,label=round(value*100,1),color=variables),show.legend = F,hjust=-0.2,size=2.8)
...
\textbf{`r paste0(meses_b[params$month]," ",params$year,"/",params$year-1,"p")`}\
...
tabla1 <- tabla1 %>% arrange(produccion)
tabla1[40,c("varprod","produccion","varventas","ventas","varpersonas","personal")]<-0
...
De las 39 actividades industriales representadas por la encuesta, un total de `r paste0(sum(tabla1$varprod>0))` registraron variaciones positivas en su producción real, sumando `r round(sum(tabla2$produccion[tabla2$varprod>0]),1)` puntos porcentuales a la variación total anual y `r paste0(sum(tabla1$varprod<0))` subsectores con variaciones negativas restaron en conjunto `r round(abs(sum(tabla2$produccion[tabla2$varprod<0])),1)` puntos porcentuales a la variación total (Tabla 1).
...
tabla_paginada[[i]] <- tabla_paginada[[i]] %>% gt(rowname_col="Clase",groupname_col = "") %>% cols_width(
    DOMINIO39_DESCRIP ~ pct(18),
    varprod_plot ~  pct(5),
    varventas_plot ~  pct(5),
    varpersonas_plot ~  pct(5),
    DOMINIO_39 ~ pct(4),
    varprod ~ pct(8),
    varventas ~ pct(8),
    varpersonas ~ pct(8),
    everything() ~ pct(9)
                     ...
                     personal =md("**Contribución<br>Personas**")) %>% tab_options(table.font.size=6,table.width = pct(65),table.layout="fixed") %>% tab_style(style=cell_text(color="white"),
                                                                                                                    locations=cells_body(rows=is.na(DOMINIO_39)==TRUE))

gtsave(tabla_paginada[[i]],paste0("tabla1_",i,".png"))
}

knitr::include_graphics(paste0("tabla1_", 1, ".png"),dpi=200)
cat("\n\n")

```

```{r tabla1_2,echo=FALSE,fig.align='center'}
knitr::include_graphics(paste0("tabla1_", 2, ".png"),dpi=200)
cat("\n\n")
```
...
\textbf{`r paste0("Enero - ",meses_b[params$month],"", params$year,"/",params$year-1,"p")`}
...
ggplot(data=variaciones_totales,aes(x=periodo,y=value,group=variables,color=variables))+geom_line(size=0.8)+
  ...
  scale_y_continuous(breaks=function(x)pretty_breaks(n=20)(c(as.numeric(x)[1],as.numeric(x)[2])),labels = percent)+
  geom_hline(aes(yintercept=0),color="gray",size=0.5)+
  geom_text(data=variaciones_totales[variaciones_totales$periodo%in%paste0(meses[params$month],"-",substr(params$year,3,4)) & variaciones_totales$variables=="Producción Real",],aes(x=periodo,y=0.11,label=round(value*100,1),color=variables),show.legend = F,hjust=0,size=2.2)+
    geom_text(data=variaciones_totales[variaciones_totales$periodo%in%paste0(meses[params$month],"-",substr(params$year,3,4)) & variaciones_totales$variables=="Ventas Reales",],aes(x=periodo,y=0.09,label=round(value*100,1),color=variables),show.legend = F,hjust=0,size=2.2)+
    geom_text(data=variaciones_totales[variaciones_totales$periodo%in%paste0(meses[params$month],"-",substr(params$year,3,4)) & variaciones_totales$variables=="Personal Ocupado",],aes(x=periodo,y=0.06,label=round(value*100,1),color=variables),show.legend = F,hjust=-0,size=2.2)+
...
\newpage
De las 39 actividades industriales representadas por la encuesta, un total de `r paste0(sum(tabla1$varprod>0))` registraron variaciones positivas en su producción real, sumando `r sum(tabla2$produccion[tabla2$varprod>0])` puntos porcentuales a la variación año corrido y `r sum(tabla1$varprod<0)` subsectores con variaciones negativas restaron en conjunto `r abs(sum(tabla1$produccion[tabla1$varprod<0]))` puntos porcentuales a la variación total (Tabla 2).
...
knitr::include_graphics(paste0("tabla2_", 1, ".png"),dpi=200)
cat("\n\n")
...
knitr::include_graphics(paste0("tabla2_", 2, ".png"),dpi=200)
cat("\n\n")
```
...
\textbf{`r paste0(meses_b[params$month+1]," ",params$year-1," - ",meses_b[params$month]," ", params$year," / ",meses_b[params$month+1]," ",params$year-2," - ",meses_b[params$month]," ", params$year-1,"P")`}\

**Gráfico 4. Variación doce meses de la producción real, ventas y personal ocupado de la industria manufacturera**
**Total nacional**
\textbf{`r paste0("Enero ",params$year-2," - ",meses_b[params$month]," ",params$year,"P")`}
...
Durante los últimos doce meses hasta `r paste0(tolower(meses_b[params$month])," de ",params$year)`, la producción real de la industria manufacturera presentó una variación de `r paste0(round(variaciones_totales[variaciones_totales$periodo%in%paste0(meses[params$month],"-",substr(params$year,3,4)) & variaciones_totales$variables=="Producción Real","value"]*100,1),"%")`, las ventas reales de `r paste0(round(variaciones_totales[variaciones_totales$periodo%in%paste0(meses[params$month],"-",substr(params$year,3,4)) & variaciones_totales$variables=="Ventas Reales","value"]*100,1),"%")` y el personal ocupado de `r paste0(round(variaciones_totales[variaciones_totales$periodo%in%paste0(meses[params$month],"-",substr(params$year,3,4)) & variaciones_totales$variables=="Personal Ocupado","value"]*100,1),"%")`.\

```{r anio_corrido_view_2,echo=F,eval=TRUE,warning=FALSE,message=F,fig.align='center',fig.height=3,fig.width=4.8}
ggplot(data=variaciones_totales,aes(x=periodo,y=value,group=variables,color=variables))+geom_line(size=0.89)+
  ...
  geom_text(data=variaciones_totales[variaciones_totales$periodo%in%paste0(meses[params$month],"-",substr(params$year,3,4)) & variaciones_totales$variables=="Producción Real",],aes(x=periodo,y=0.11,label=round(value*100,1),color=variables),show.legend = F,hjust=0,size=2.2)+
    geom_text(data=variaciones_totales[variaciones_totales$periodo%in%paste0(meses[params$month],"-",substr(params$year,3,4)) & variaciones_totales$variables=="Ventas Reales",],aes(x=periodo,y=0.09,label=round(value*100,1),color=variables),show.legend = F,hjust=0,size=2.2)+
    geom_text(data=variaciones_totales[variaciones_totales$periodo%in%paste0(meses[params$month],"-",substr(params$year,3,4)) & variaciones_totales$variables=="Personal Ocupado",],aes(x=periodo,y=0.06,label=round(value*100,1),color=variables),show.legend = F,hjust=-0,size=2.5)+
  theme(strip.background.x=element_rect(color = NA,  fill=NA),strip.text = element_text(color="transparent"))
...
tabla1["varventas_por"] <- round((abs(tabla1$varventas)/max(tabla1$varventas))*100,1)
tabla1["varpersonas_por"] <- round((abs(tabla1$varpersonas)/max(tabla1$varpersonas))*100,1)
#corregir suma de valores positivos
#Corregir los ma de las tablas 
```

De las 39 actividades industriales representadas por la encuesta, un total de `r sum(tabla1$varprod>0)` registraron variaciones positivas en su producción real, sumando `r round(sum(tabla2$produccion[tabla2$varprod>0]),1)` puntos porcentuales a la variación total doce meses y `r sum(tabla1$varprod<0)` subsectores con variaciones negativas restaron en conjunto `r round(abs(sum(tabla2$produccion[tabla2$varprod<0])),1)` puntos porcentuales a la variación total. (Tabla 3)\
...
knitr::include_graphics(paste0("tabla3_", 1, ".png"),dpi=200)
cat("\n\n")
...
knitr::include_graphics(paste0("tabla3_", 2, ".png"),dpi=200)
cat("\n\n")
```
...
tabla1["varprod_por"] <- abs(round((abs(tabla1$varprod)/max(tabla1$varprod))*100,1))
tabla1["varventas_por"] <- abs(round((abs(tabla1$varventas)/max(tabla1$varventas))*100,1))
tabla1["varpersonas_por"] <- abs(round((abs(tabla1$varpersonas)/max(tabla1$varpersonas))*100,1))

```


```{r, eval=TRUE, echo=FALSE}
if(sum(tabla1$varprod>0)==0){a <- paste0("Los 14 dominios de departamentos representados por la encuesta, registraron variaciones negativas en su producción real, sumando ", abs(round(sum(tabla2$produccion[tabla2$varprod<0]),1))," p.p. a la variación total nacional.")}else if (sum(tabla1$varprod<0)==0){a <- paste0("Los 14 dominios de departamentos representados por la encuesta, registraron variaciones positivas en su producción real, sumando ", abs(round(sum(tabla2$produccion[tabla2$varprod>0]),1))," p.p. a la variación total nacional.")}else if(sum(tabla2$produccion)<0){a <- paste0("De los 14 dominios de departamentos representados por la encuesta, ", sum(tabla1$varprod>0), " registraron variaciones positivas en su producción real, sumando ", round(sum(tabla2$produccion[tabla2$varprod>0]),1), " p.p. a la variación total nacional.")}else if(sum(tabla2$produccion)>=0){a <- paste0("De los 14 dominios de departamentos representados por la encuesta, ", sum(tabla1$varprod<0), "registraron variaciones negativas en su producción real, sumando ", round(sum(tabla2$produccion[tabla2$varprod<0]),1), " p.p. a la variación total nacional.")}

```

<!-- `r paste0("Los 14 dominios de departamentos representados por la encuesta,"     ,ifelse(sum(tabla1$varprod>0)==0,"registraron variaciones negativas",ifelse(sum(tabla1$varprod<0)==0,"registraron variaciones positivas",ifelse(sum(tabla2$produccion)<0,paste0( sum(tabla1$varprod>0), " registraron variaciones positivas"),ifelse(sum(tabla2$produccion)>=0,paste0(sum(tabla1$varprod<0), "registraron variaciones negativas"))))),"en su producción real, sumando ", abs(round(sum(tabla2$produccion[tabla2$varprod<0]),1))," p.p. a la variación total nacional.")` -->


`r if(sum(tabla1$varprod>0)==0){a <- paste0("Los 14 dominios de departamentos representados por la encuesta, registraron variaciones negativas en su producción real, restando ", abs(round(sum(tabla2$produccion[tabla2$varprod<0]),1))," p.p. a la variación total nacional.")}else if (sum(tabla1$varprod<0)==0){a <- paste0("Los 14 dominios de departamentos representados por la encuesta, registraron variaciones positivas en su producción real, sumando ", abs(round(sum(tabla2$produccion[tabla2$varprod>0]),1))," p.p. a la variación total nacional.")}else if(sum(tabla2$produccion)<0){ a <- paste0("De los 14 dominios de departamentos representados por la encuesta, ", sum(tabla1$varprod>0), " registraron variaciones positivas en su producción real, sumando ", round(sum(tabla2$produccion[tabla2$varprod>0]),1), " p.p. a la variación total nacional.")}else if(sum(tabla2$produccion)>=0){a <- paste0("De los 14 dominios de departamentos representados por la encuesta, ", sum(tabla1$varprod<0), "registraron variaciones negativas en su producción real, sumando ", round(sum(tabla2$produccion[tabla2$varprod<0]),1), " p.p. a la variación total nacional.")}`

`r a`

<!-- De los 14 dominios de departamentos representados por la encuesta, `r sum(tabla1$varprod>0)` registraron variaciones positivas en su producción real, sumando `r round(sum(tabla2$produccion),1)` p.p. a la variación total nacional.\ -->
...
tabla_paginada[[i]] <- tabla_paginada[[i]] %>% gt(rowname_col="Clase",groupname_col = "") %>% cols_width(
    INCLUSION_NOMBRE_DEPTO ~ pct(12),
    varprod_plot ~  pct(6),
    varventas_plot ~  pct(6),
    varpersonas_plot ~  pct(6),
    varprod ~ pct(9),
    varventas ~ pct(9),
    varpersonas ~ pct(9),
    everything() ~ pct(10)
                     ...
                     varpersonas_plot ="",
                     varpersonas =md("**Variación<br>Personas**"),
                     personal =md("**Contribución<br>Personas**")) %>% tab_options(table.font.size=8,table.width = pct(50),table.layout="fixed") %>% tab_style(style=cell_text(color="white"),
                                                                                                                    locations=cells_body(rows=varprod==0.001))

gtsave(tabla_paginada[[i]],paste0("tabla4_",i,".png"))
}
knitr::include_graphics(paste0("tabla4_", 1, ".png"),dpi=200)
...
<!-- En `r paste0(tolower(meses_b[params$month])," de ",params$year)` frente a `r paste0(tolower(meses_b[params$month])," ",params$year-1)`, el área metropolitana que más constribuye a la variación anual de la producción real es la de `r tabla2$AREA_METROPOLITANA[which.max(abs(tabla2$varprod))]` con una variación de `r tabla2$varprod[which.max(abs(tabla2$varprod))]`%, sumando `r abs(tabla2$produccion[which.max(abs(tabla2$varprod))])` p.p. a la variación total nacional.\ -->
\
...
knitr::include_graphics(paste0("tabla5_", 1, ".png"),dpi=150)
...
<!-- En `r paste0(tolower(meses_b[params$month])," de ",params$year)` frente a `r paste0(tolower(meses_b[params$month])," de ",params$year-1)` de los 10 dominios de ciudades representados por la encuesta, `r sum(tabla2$varprod>0)` registraron variaciones positivas en su producción real, sumando `r sum(tabla2$produccion[tabla2$varprod>0])` p.p. de la variación total nacional (Tabla 6)\ -->
...
knitr::include_graphics(paste0("tabla6_", 1, ".png"),dpi=200)
...
knitr::include_graphics(paste0("tabla7_", 1, ".png"),dpi=200)
...
knitr::include_graphics(paste0("tabla8_", 1, ".png"),dpi=200)
...
knitr::include_graphics(paste0("tabla9_", 1, ".png"),dpi=200)
...
knitr::include_graphics(paste0("tabla10_", 1, ".png"),dpi=200)
...
knitr::include_graphics(paste0("tabla11_", 1, ".png"),dpi=200)
...
knitr::include_graphics(paste0("tabla12_", 1, ".png"),dpi=200)
...
En `r paste0(tolower(meses_b[params$month])," de ",params$year)` frente al mismo mes de 2019, la producción real de la industria manufacturera presentó una variación de `r paste0(round(variacion$varprod2019[variacion$ANIO==params$year]*100,1)," %")`, las ventas reales de `r paste0(round(variacion$varventa2019[variacion$ANIO==params$year]*100,1)," %")` y el personal ocupado   `r paste0(round(variacion$varpersonas2019[variacion$ANIO==params$year]*100,1)," %")`.\
...
```{r variacion_trienal_plot,echo=F,eval=TRUE,warning=FALSE,message=F,fig.align='center',fig.height=3.3,fig.width=3.8}
...
De las 39 actividades industriales representadas por la encuesta, un total de `r sum(tabla1$varprod>0)` registraron variaciones positivas en su producción real, sumando `r round(sum(tabla2$produccion[tabla2$varprod>0]),1)` puntos porcentuales a la variación trienal y `r sum(tabla1$varprod<0)` subsectores con variaciones negativas restaron en conjunto `r abs(round(sum(tabla2$produccion[tabla2$varprod<0]),1))` puntos porcentuales a la variación total. (Tabla 13).\
\newpage
...
tabla_paginada[[i]] <- tabla_paginada[[i]] %>% gt(rowname_col="Clase",groupname_col = "") %>% cols_width(
    ...
    everything() ~ pct(9)
                     ...
                     varpersonas_plot ="",
                     varpersonas =md("**Variación<br>Personas**"),
                     personal =md("**Contribución<br>Personas**")) %>%
  tab_options(table.font.size=6,table.width = pct(68),table.layout="fixed") %>%
...
knitr::include_graphics(paste0("tabla13_", 1, ".png"),dpi=200)
cat("\n\n")
...
knitr::include_graphics(paste0("tabla13_", 2, ".png"),dpi=200)
cat("\n\n")
```
...
\textbf{`r paste0(meses_b[params$month], " (",params$year,"/",2019,")P")`}
...
<!-- De los 14 dominios de departamentos representado por la encuesta, `r sum(tabla1$varprod>0)` registraron variaciones positivas en su producción real, sumando `r sum(tabla1$produccion[tabla1$produccion>0])` p.p. a la variación total nacional.\ -->
...
knitr::include_graphics(paste0("tabla14_", 1, ".png"),dpi=200)
...
knitr::include_graphics(paste0("tabla15_", 1, ".png"),dpi=200)
...
<!-- En `r paste0(tolower(meses_b[params$month])," de ",params$year)` frente al mismo mes de 2019 de los 10 dominios de ciudades representados por la encuesta, `r sum(tabla1$varprod>=0)` registraron variaciones positivas en su producción real, sumando `r sum(tabla1$produccion[tabla1$varprod>0])` p.p. de la variación total nacional. (Tabla 16)\ -->
...
knitr::include_graphics(paste0("tabla16_", 1, ".png"),dpi=200)
...
\begin{align*}
ICProducción &= `r params$IC_prod`\%\\
ICVentas &= `r params$IC_ven`\%\\
ICEmpleo &= `r params$IC_empl`\%\\
\end{align*}
...
\begin{align*}
TIProducción &= `r params$TI_prod` \%\\
TIPVentas &= `r params$TI_ven` \%\\
TIEmpleo &= `r params$TI_empl`\%\\
\end{align*}
