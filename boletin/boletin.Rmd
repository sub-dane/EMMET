...
header-includes: 
  ...
  - \fancyhead[R]{\includegraphics[width=18cm]{img/header.png}\\
     ...
     \textcolor{gray}{\Large `r paste(params$month_b,params$year)`}}}
...
data<-read.csv(paste0(params$directorio,"/results/S5_tematica/EMMET_PANEL_tematica_",meses[params$month],params$year,".csv"))
...
```{r variacion,echo=F,eval=TRUE,warning=FALSE,message=F,fig.align='center',fig.ext='pdf'}
#1
variacion <- data %>% 
  group_by(ANIO,MES)
...
num_rows_per_page <- 13  # Define el número de filas por página
...
for (i in 1:num_pages) {
  ...
  tabla_paginada[[i]] <- tabla1[start_row:end_row, ] %>%
    mutate(varprod_plot = purrr::map(varprod_por, 
     ...
     select(DOMINIO_39,DESCRIPCIONDOMINIOEMMET39,varprod,varprod_plot,produccion,varventas,varventas_plot,ventas,varpersonas,varpersonas_plot,personal) 

  
tabla_paginada[[i]] <- tabla_paginada[[i]] %>% gt(rowname_col="Clase",groupname_col = "") %>% cols_width(
    DESCRIPCIONDOMINIOEMMET39 ~ px(140),
    varprod_plot ~  px(30),
    ...
    everything() ~ px(48)
                     ...
                     DESCRIPCIONDOMINIOEMMET39=md("**Descripción**"),
                     DOMINIO_39 =md("**Clase**"),
...
```{r tabla1_3,echo=FALSE,fig.align='center'}
knitr::include_graphics(paste0("tabla1_", 3, ".png"))
```
...
```{r tabla2_3,echo=FALSE,fig.align='center'}
knitr::include_graphics(paste0("tabla2_", 3, ".png"))
```
...
```{r tabla3_3,echo=FALSE,fig.align='center'}
knitr::include_graphics(paste0("tabla3_", 3, ".png"))
```
...
```{r tabla13_3,echo=FALSE,fig.align='center'}
knitr::include_graphics(paste0("tabla13_", 3, ".png"))
```
